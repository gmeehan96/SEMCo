...
class ColdStartDataBuilder(object):
    def __init__(
        ...
        self.mapped_cold_test_item_idx = self.get_item_id_list(
            list(self.cold_test_set_item)
        )
        self.mapped_cold_valid_item_idx = self.get_item_id_list(
            list(self.cold_valid_set_item)
        )

        all_item_idx = set(
            np.concatenate([self.mapped_warm_item_idx, self.mapped_cold_item_idx])
        )
        self.item_mask = np.array(
            [0 if x in all_item_idx else -10e8 for x in range(self.item_num)]
        )
    ...
    def warm_valid_size(self):
        return (
            len(self.warm_valid_set),
            len(self.warm_valid_set_item),
            len(self.warm_valid_data),
        )

    def warm_test_size(self):
        return (
            len(self.warm_test_set),
            len(self.warm_test_set_item),
            len(self.warm_test_data),
        )

    def cold_valid_size(self):
        return (
            len(self.cold_valid_set),
            len(self.cold_valid_set_item),
            len(self.cold_valid_data),
        )

    def cold_test_size(self):
        return (
            len(self.cold_test_set),
            len(self.cold_test_set_item),
            len(self.cold_test_data),
        )
...
class ColdStartDataBuilder_MultiItemContent(object):
    def __init__(
        self,
        training_data,
        warm_valid_data,
        ...
        super(ColdStartDataBuilder_MultiItemContent, self).__init__()
        self.training_data = training_data
        self.warm_valid_data = warm_valid_data
        self.warm_test_data = warm_test_data
        self.cold_valid_data = cold_valid_data
        ...
        if item_content is not None:
            self.source_item_content = item_content
            self.mapped_item_content = [
                np.empty((ic.shape[0], ic.shape[1]), dtype=np.float32)
                for ic in item_content
            ]
            self.item_content_dim = [ic.shape[-1] for ic in item_content]
    ...
    def generate_set(self):
        ...
        for entry in self.training_data:
            ...
            if item not in self.item:
                ...
                self.id2item[self.item[item]] = item
                if self.source_item_content is not None:
                    for i, ic in enumerate(self.mapped_item_content):
                        ic[self.item[item]] = self.source_item_content[i][item]
                # userList.append
            self.training_set_u[user][item] = rating
            self.training_set_i[item][user] = rating
